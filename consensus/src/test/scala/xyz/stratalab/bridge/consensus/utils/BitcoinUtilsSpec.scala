...
class BitcoinUtilsSpec extends CatsEffectSuite {
  ...
  test("Fee Estimation > Verify specified Fee Rate is used in transaction") {
    implicit val btcWaitExpirationTime: BTCWaitExpirationTime =
      new BTCWaitExpirationTime(1000)
    ...
    val dummyHash =
      ByteVector(MessageDigest.getInstance("SHA-256").digest("secret".getBytes))
    val escrowScript = buildScriptAsm(
    ...
    val sequence: UInt32 =
      UInt32(1000L & TransactionConstants.sequenceLockTimeMask.toLong)
    val dummyInput = TransactionInput(
      TransactionOutPoint(DoubleSha256DigestBE.empty, 0),
      ScriptSignature.empty,
      sequence
    )
    val dummyOutput =
      TransactionOutput(desiredAmt, P2WPKHWitnessSPKV0(ECPublicKey.dummy))

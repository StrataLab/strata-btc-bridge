...
object BitcoinUtils {
  ...
  def calculateBtcReclaimFee(
    tx:         Transaction,
    feePerByte: FeeUnit
  ): CurrencyUnit = feePerByte.calc(tx)

  /**
   * Estimate the reclaim fee for a future reclaim transaction. This fee must
   * be deposited by the user to the escrow address in addition to the BTC that
   * is meant to be wrapped. This fee will be used when the escrow address is
   * being spent from; for example, during user reclaim (sad path).
   *
   * Virtual bytes include witness data, so we must include the signatures in
   * the transaction prior to calculating
   *
   * This is an estimation since we use dummy values for any unavailable
   * information (txOut reference, keys, etc)
   */
  def estimateBtcReclaimFee(
    ...
    val dummyScript = RawScriptPubKey(
      buildScriptAsm(
        ...
        ECPublicKey.dummy,
        ByteVector(
          MessageDigest.getInstance("SHA-256").digest("dummy".getBytes)
        ),
        btcWaitExpirationTime.underlying
      )
    )
    val serializedTxForSignature = serializeForSignature(
      dummyUnprovenTx,
      inputAmount.satoshis,
      dummyScript.asm
    )
    ...
    val userSignature = ECDigitalSignature(
      dummyUserPrivKey.sign(signableBytes).bytes ++ ByteVector.fromByte(
